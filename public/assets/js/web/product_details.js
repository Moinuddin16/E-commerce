    ...
    n = (function () {
        function t(t, n) {
            ...
            this.$el.on("mouseover.starrr", "span", function (e) {
                return o.syncRating(
                    o.$el.find("span").index(e.currentTarget) + 1
                );
            });
...
$(document).ready(function () {
    $(document).on('submit', '#submit-review-from', function (e) {
        e.preventDefault();
        var form = $(this);
    ...
    function makeReview(res) {
        baseUrl = $("#baseUrl").val();
        let ratingCount = res.review.rating;
        let rating = ``;
        for(let i = 1; i <= 5; i++) 
        {
            rating += `<span class="fa fa-star ${i > ratingCount ? '' : 'checked'}"></span>`;
        }
        var review = 
        `
            <div class="col-lg-1" style="margin-right: -25px !important">
                <img src="${baseUrl}/public/assets/avater.png" alt="" width="50px" height="50px">
            </div>
            <div class="col-lg-11">
                <h5><strong>${res.username}</strong></h5>
                <div class="">
                    ${rating}
                </div>
                <p >${res.review.review}</p>
            </div>
        <hr>
        `;
        return review;
